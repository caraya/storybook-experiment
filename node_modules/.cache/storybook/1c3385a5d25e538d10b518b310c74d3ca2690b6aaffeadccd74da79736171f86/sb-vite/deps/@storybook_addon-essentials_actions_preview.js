...
var logActionsWhenMockCalled = (context) => {
  ...
  if (!(actions == null ? void 0 : actions.disable) && !subscribed && "__STORYBOOK_TEST_ON_MOCK_CALL__" in import_global.global && typeof import_global.global.__STORYBOOK_TEST_ON_MOCK_CALL__ == "function") {
    ...
    onMockCall((mock, args) => {
      ...
      (!/^next\/.*::/.test(name) || ["next/router::useRouter()", "next/navigation::useRouter()", "next/navigation::redirect", "next/cache::", "next/headers::cookies().set", "next/headers::cookies().delete", "next/headers::headers().set", "next/headers::headers().delete"].some((prefix) => name.startsWith(prefix))) && action(name)(args);
    }), subscribed = true;
  }
};
